...
def main():
    ...
    lars_data = get_swapi_resource(f"{ENDPOINT}/people/", {'search': 'owen lars'})['results'][0]
    print(f"Owen Lars data = {lars_data}")


    # Retrieve Owen Lars's species and assign to < lars_species >
    lars_species_url = lars_data['species'][0]
    lars_species = get_swapi_resource(lars_species_url)['name']
    lars_data.update({'species_name': lars_species})
    print(f"Owen Lars data w/Species = {lars_data}")


    # Update values in < lars >.
    for key, val in lars_data.items():
        for key2 in lars:
            if key == key2:
                lars.update({key: val})
